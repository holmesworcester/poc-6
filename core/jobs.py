...
class SyncUpdateJob(Job):
    """Materialize ingest log, update negentropy buckets, and handle protocol messages.

    Processes in order to ensure blobs are in negentropy buckets before
    protocol messages are handled. This guarantees hashes are current when
    responding to range_requests.

    Order per batch:
    1. Materialize all blobs (store to DB)
    2. Add all blobs to negentropy buckets (update hashes)
    3. Handle all protocol messages (see current hashes)
    """
    ...
    def run(self, t_ms: int, db: Any) -> dict:
        ...
        while True:
            ...
            # Step 2: Add blobs to negentropy BEFORE handling protocol
            # This ensures hashes are current when we respond to range_requests
            if shareable_rows:
            ...
            # Step 3: Handle protocol messages (hashes now current)
        ...
        db.commit()
        return {'materialized': processed, 'protocol_handled': protocol_handled}
