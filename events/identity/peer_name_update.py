...
def project(event_id: str, recorded_by: str, recorded_at: int, db: Any) -> str | None:
    ...
    try:
        ...
        log.warning(f"peer_name_update.project() failed to parse event: {e}")
        return None
    ...
    if not peer_target_id or not encrypted_name:
    ...
    try:
        ...
        if decrypted_name is not None:
            ...
            log.info(f"peer_name_update.project() decrypted peer name for {peer_target_id[:20]}...: {decrypted_name}")
            ...
            if existing is None:
                ...
                safedb.execute(
                    """INSERT INTO peer_names
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (peer_target_id, decrypted_name, event_id, global_count, key_id,
            ...
            log.info(f"peer_name_update.project() stored decrypted peer name")
        else:
            # Key not available yet - store encrypted blob in pending table
            log.info(f"peer_name_update.project() key not available yet, storing in pending table")
            import hashlib
            ...
            safedb.execute(
                """INSERT OR IGNORE INTO pending_name_decrypts
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (pending_id, 'peer_name', peer_target_id, event_id, encrypted_name, key_id,
                 'waiting_for_key', event_data.get('created_at'), recorded_by, recorded_at)
            )
            log.info(f"peer_name_update.project() stored in pending_name_decrypts")

    except Exception as e:
        log.warning(f"peer_name_update.project() decryption error: {e}")
