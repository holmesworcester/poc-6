...
def list_local(db: Any) -> list[str]:
    """List all local peer IDs.

    Returns:
        List of peer_id strings for all local peers (accounts on this device)
    """
    unsafedb = create_unsafe_db(db)
    rows = unsafedb.query("SELECT peer_id FROM local_peers ORDER BY created_at ASC")
