...
def create(inviter_peer_id: str, inviter_peer_shared_id: str,
    ...
    # Use synthetic ID for invite prekey (no encrypted event - direct prekeys_shared insert)
    # This reuses the invite proof key for both proof signature and key wrapping
    invite_prekey_public_key_b64 = invite_pubkey_b64  # Reuse proof key
    invite_prekey_shared_id = crypto.b64encode(crypto.hash(invite_public_key, size=16))

    # Add invite prekey to Alice's prekeys_shared (direct insert, no encrypted event)
    unsafedb = create_unsafe_db(db)
    safedb = create_safe_db(db, recorded_by=inviter_peer_id)
    safedb.execute(
        "INSERT OR IGNORE INTO prekeys_shared (prekey_shared_id, peer_id, public_key, created_at, recorded_by, recorded_at) VALUES (?, ?, ?, ?, ?, ?)",
        (invite_prekey_shared_id, inviter_peer_shared_id, invite_public_key, t_ms, inviter_peer_id, t_ms)
    )
    ...
    invite_transit_key_id = key.create_with_material(
    ...
    invite_group_key_id = key.create_with_material(
    ...
    # Query prekey with public key and prekey_shared_id from prekeys table
    inviter_prekey_row = unsafedb.query_one(
        "SELECT prekey_id, public_key, prekey_shared_id FROM prekeys WHERE owner_peer_id = ? ORDER BY created_at DESC LIMIT 1",
        (inviter_peer_id,)
    )
    ...
    inviter_prekey_id = inviter_peer_id  # Use peer_id as prekey identifier
    inviter_prekey_public_key = inviter_prekey_row['public_key']  # Raw bytes from DB
    inviter_prekey_shared_id = inviter_prekey_row['prekey_shared_id']  # Event ID for projection
    ...
    invite_event_data = {
        ...
        'invite_prekey_shared_id': invite_prekey_shared_id,  # Event ID for invite prekey
        ...
        'inviter_prekey_shared_id': inviter_prekey_shared_id,  # Event ID for inviter's prekey
    ...
    network_key_row = unsafedb.query_one(
        "SELECT key FROM keys WHERE key_id = ?",
        (key_id,)
    )
    ...
    # key_id is already an event ID (from key.create()), use it directly as hint
...
def project(invite_id: str, recorded_by: str, recorded_at: int, db: Any) -> str | None:
    ...
    # Bob reconstructs this locally from invite event data (doesn't need to decrypt prekey_shared blob)
    # Alice restores it during reprojection so she can decrypt invite_key_shared from Bob
    if 'invite_prekey_public_key' in event_data and 'invite_prekey_shared_id' in event_data:
        invite_prekey_public_key_bytes = crypto.b64decode(event_data['invite_prekey_public_key'])
        safedb.execute(
            "INSERT OR REPLACE INTO prekeys_shared (prekey_shared_id, peer_id, public_key, created_at, recorded_by, recorded_at) VALUES (?, ?, ?, ?, ?, ?)",
            (event_data['invite_prekey_shared_id'], created_by, invite_prekey_public_key_bytes, event_data['created_at'], recorded_by, recorded_at)
        )

    # Project inviter's prekey into prekeys_shared (for Bob to send sync requests to Alice)
    # Always project for anyone who receives the invite (INSERT OR IGNORE handles duplicates)
    if 'inviter_prekey_public_key' in event_data and 'inviter_peer_shared_id' in event_data and 'inviter_prekey_shared_id' in event_data:
        ...
        safedb.execute(
            "INSERT OR IGNORE INTO prekeys_shared (prekey_shared_id, peer_id, public_key, created_at, recorded_by, recorded_at) VALUES (?, ?, ?, ?, ?, ?)",
            (event_data['inviter_prekey_shared_id'], inviter_peer_shared_id, inviter_prekey_public_key_bytes, event_data['created_at'], recorded_by, recorded_at)
        )
