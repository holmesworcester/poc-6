...
def project_ids(recorded_ids: list[str], db: Any) -> list[list[str | None]]:
    ...
    log.info(f"recorded.project_ids() projecting {len(recorded_ids)} recorded events")
    projected_ids = [project(id, db) for id in recorded_ids]
    log.info(f"recorded.project_ids() completed projection of {len(recorded_ids)} events")
    return projected_ids


def project(recorded_id: str, db: Any) -> list[str | None]:
    ...
    if event_type == 'message':
        ...
        key.project(ref_id, recorded_by, db)
        projected_id = ref_id
        ...
        projected_id = prekey_shared.project(ref_id, recorded_by, recorded_at, db)
