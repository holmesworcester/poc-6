"""
Performance test: Alice creates 1,000 messages, Bob syncs them.
...
@pytest.mark.slow
def test_sync_perf_1k(fresh_db):
    """Perf gut-check: sync 1k messages between two peers.

    With bisection protocol: ~100 rounds, ~0.5s
    """
    db = fresh_db
    num_messages = 1000
    ...
    print("Syncing...")
    assert_eventually(bob_has_all_messages, db=db, start_t_ms=None, max_rounds=150)
...
def test_sync_perf_10k(fresh_db):
    """Perf: sync 10k messages between two peers.

    With bulk transfer optimization: should complete efficiently
    """
    db = fresh_db
    num_messages = 10000
    ...
    print("Syncing...")
    assert_eventually(bob_has_all_messages, db=db, start_t_ms=None, max_rounds=300)
