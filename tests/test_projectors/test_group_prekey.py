"""Tests for group_prekey.create_pure() - local-only keypair."""
import crypto
from projectors import compute_event_id
from projectors.group_prekey import create_pure


class TestGroupPrekeyCreatePure:
    """Test group_prekey.create_pure()."""
    ...
    def test_local_only_keypair(self):
        """Group prekey stores keypair for receiving sealed keys."""
        deps = self._make_deps()
        result = create_pure(deps, 1000)

        blob_data = crypto.parse_json(result.blobs[0].blob)
        assert blob_data['type'] == 'group_prekey'
    ...
    def test_event_type_correct(self):
        """Event type is group_prekey."""
        deps = self._make_deps()
        result = create_pure(deps, 1000)

        assert result.blobs[0].event_type == 'group_prekey'
