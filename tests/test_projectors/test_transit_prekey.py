"""Tests for transit_prekey.create_pure() - local-only keypair."""
import crypto
from projectors import compute_event_id
from projectors.transit_prekey import create_pure


class TestTransitPrekeyCreatePure:
    """Test transit_prekey.create_pure() - local-only."""
    ...
    def test_event_type_correct(self):
        """Event type is transit_prekey."""
        deps = self._make_deps()
        result = create_pure(deps, 1000)

        assert result.blobs[0].event_type == 'transit_prekey'
