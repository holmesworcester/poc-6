...
def _run_scenario() -> None:
    ...
    try:
        ...
        print("✓ SUCCESS: Bob cannot create channel")
    ...
    try:
        ...
        print("✓ SUCCESS: Alice created public channel")
    except Exception as exc:
        print(f"❌ FAILURE: Alice could not create public channel: {exc}")
    ...
    try:
        ...
        print("✓ SUCCESS: Alice created private channel for Bob")
    except Exception as exc:
        print(f"❌ FAILURE: Alice could not create private channel: {exc}")
        return
    ...
    if private_ch:
        ...
        if alice_is_member:
            print("✓ SUCCESS: Alice (admin/creator) is a member of the private group")
        else:
            print("❌ FAILURE: Alice is not a member of her own private group")
    else:
        print("❌ FAILURE: Could not find private channel group")
    ...
    try:
        ...
        print("✓ SUCCESS: Alice added Charlie to private channel (event created)")
    except Exception as exc:
        print(f"❌ FAILURE: Could not add Charlie: {exc}")
    ...
    try:
        ...
        print("✓ SUCCESS: Bob cannot add members")
    ...
    if public_ch and private_ch:
        if public_ch['group_id'] != private_ch['group_id']:
            print("✓ SUCCESS: Public and private channels use different groups")
            ...
            if public_group_key and private_group_key:
                if public_group_key['key_id'] != private_group_key['key_id']:
                    print("✓ SUCCESS: Channels use different encryption keys")
                else:
                    print("❌ FAILURE: Channels use the same encryption key")
        else:
            print("❌ FAILURE: Both channels use the same group!")
