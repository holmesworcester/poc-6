...
def test_peer_view_isolation():
    ...
    # Alice inserts a message (11 columns: message_id, channel_id, group_id, author_id, signed_by, content, created_at, ttl_ms, key_id, recorded_by, recorded_at)
    alice_db.execute(
        "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ('msg1', 'chan1', 'grp1', alice_id, alice_id, 'Alice secret', 1000, 0, None, alice_id, 1001)
    )

    # Bob inserts a message with same message_id (different peer view)
    bob_db.execute(
        "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ('msg1', 'chan1', 'grp1', bob_id, bob_id, 'Bob secret', 2000, 0, None, bob_id, 2001)
    )
...
def test_safedb_catches_wrong_recorded_by_in_results():
    ...
    db.execute(
        "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ('msg1', 'chan1', 'grp1', alice_id, alice_id, 'Alice msg', 1000, 0, None, alice_id, 1001)
    )
    db.execute(
        "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ('msg2', 'chan1', 'grp1', bob_id, bob_id, 'Bob msg', 2000, 0, None, bob_id, 2001)
    )
    db.commit()
...
def test_safedb_insert_validation():
    ...
    with pytest.raises(ScopingViolation, match="must include recorded_by"):
        alice_db.execute(
            "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ('msg1', 'chan1', 'grp1', 'author', 'signer', 'content', 1000, 0, None, 'bob', 1001)  # Wrong peer!
        )

    # Should accept INSERT with correct recorded_by
    alice_db.execute(
        "INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ('msg1', 'chan1', 'grp1', 'author', 'signer', 'content', 1000, 0, None, 'alice', 1001)
    )
    db.commit()
